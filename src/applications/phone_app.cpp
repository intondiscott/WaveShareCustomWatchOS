...
{
  if (!lv_obj_is_valid(SmartWatchUI_t.phone))
  {
    SmartWatchUI_t.phone = lv_obj_create(lv_screen_active());
    lv_obj_t *label = lv_label_create(SmartWatchUI_t.phone);
    SmartWatchUI_t.close_btn = lv_button_create(SmartWatchUI_t.phone);
    ...
    lv_obj_set_size(SmartWatchUI_t.phone, TFT_WIDTH-20, TFT_HEIGHT-60);
    lv_obj_center(SmartWatchUI_t.phone);
